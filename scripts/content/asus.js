const prodID = window.location.href;
if (prodID) {
    let descBox = document.getElementsByClassName('product attribute overview')[0];
    ...
    if (descBox) {
        ...
        chrome.storage.local.get(['user_id', 'session_id', prodIDKey], async function (stored) {
            ...
            document.getElementsByClassName('product media')[0].appendChild(controlBox);
